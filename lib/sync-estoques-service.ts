 ...
 */
async function buscarEstoquesSankhya(idSistema: number): Promise<EstoqueSankhya[]> {
...
export async function sincronizarEstoquesPorEmpresa(idSistema: number, empresaNome: string): Promise<SyncResult> {
  ...
  try {
    console.log(`🚀 [Sync] Iniciando sincronização de estoques - ID_SISTEMA: ${idSistema}, Empresa: ${empresaNome}`);

    await obterToken(idSistema);
    const estoques = await buscarEstoquesSankhya(idSistema);
