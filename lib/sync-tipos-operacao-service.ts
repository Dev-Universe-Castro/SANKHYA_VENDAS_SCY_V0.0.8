 ...
 */
async function buscarTiposOperacaoSankhya(idSistema: number): Promise<TipoOperacaoSankhya[]> {
  const PAYLOAD = {
    "requestBody": {
      "dataSet": {
...
export async function sincronizarTiposOperacaoPorEmpresa(idSistema: number, empresaNome: string): Promise<SyncResult> {
  ...
  try {
    console.log(`🚀 [Sync] Iniciando sincronização de tipos de operação - ID_SISTEMA: ${idSistema}, Empresa: ${empresaNome}`);

    await obterToken(idSistema);
    const tiposOperacao = await buscarTiposOperacaoSankhya(idSistema);
