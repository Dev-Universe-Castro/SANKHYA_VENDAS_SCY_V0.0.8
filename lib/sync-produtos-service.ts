 ...
 */
async function buscarProdutosSankhya(idSistema: number): Promise<ProdutoSankhya[]> {
  const PAYLOAD = {
    "requestBody": {
      "dataSet": {
        "rootEntity": "Produto",
...
export async function sincronizarProdutosPorEmpresa(idSistema: number, empresaNome: string): Promise<SyncResult> {
  ...
  try {
    console.log(`🚀 [Sync] Iniciando sincronização de produtos - ID_SISTEMA: ${idSistema}, Empresa: ${empresaNome}`);

    await obterToken(idSistema);
    const produtos = await buscarProdutosSankhya(idSistema);
