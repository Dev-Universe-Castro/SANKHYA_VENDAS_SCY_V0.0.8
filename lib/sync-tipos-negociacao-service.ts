 ...
 */
async function buscarTiposNegociacaoSankhya(idSistema: number): Promise<TipoNegociacaoSankhya[]> {
  const PAYLOAD = {
    "requestBody": {
      "dataSet": {
...
export async function sincronizarTiposNegociacaoPorEmpresa(idSistema: number, empresaNome: string): Promise<SyncResult> {
  ...
  try {
    console.log(`🚀 [Sync] Iniciando sincronização de tipos de negociação - ID_SISTEMA: ${idSistema}, Empresa: ${empresaNome}`);
    console.log('📋 [Sync] Parâmetros recebidos:', { idSistema, empresaNome, tipo: typeof idSistema });

    await obterToken(idSistema);
    const tiposNegociacao = await buscarTiposNegociacaoSankhya(idSistema);
